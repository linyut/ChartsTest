...
{
    [super viewDidLoad];
    [self configTitle:self.VCTitle showLeftButton:YES andrightBtnTitle:nil];
...
{
    chartView.usePercentValuesEnabled = YES;//是否将数据转换百分比
    chartView.drawSlicesUnderHoleEnabled = NO;
    chartView.holeRadiusPercent = 0.58;//空心半径占比
    chartView.transparentCircleRadiusPercent = 0.61;//半透明空心占比
    chartView.chartDescription.enabled = NO;
    [chartView setExtraOffsetsWithLeft:5.f top:10.f right:5.f bottom:5.f];//饼状图距离边缘的间隙
    
    chartView.drawCenterTextEnabled = YES;//圆心是否显示文字
    ...
    chartView.centerAttributedText = centerText;//圆心文字
    
    chartView.drawHoleEnabled = YES;//饼状图是否空心
